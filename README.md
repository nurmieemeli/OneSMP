...
| [MiniPlaceholders-LuckPerms expansion](https://github.com/MiniPlaceholders/MiniPlaceholders) | ➖ | Optional — install alongside MiniPlaceholders if you want `<luckperms_prefix>`/`<luckperms_suffix>` to resolve in chat, nametags, and the scoreboard. CanvasSuite never talks to LuckPerms directly |
...
- `/world tp <name>` teleports to a world's spawn (goes through the normal warmup/`/back`-recording path, unlike `/spawn`).
...
2. Install [MiniPlaceholders](https://modrinth.com/plugin/miniplaceholders) (required); optionally install PacketEvents (nametags/spectate/reserved-slot tablist), the MiniPlaceholders-LuckPerms expansion (prefix/suffix placeholders), and Vault.
